...
function Home() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get('http://localhost:9999/users/count-user-status'); // Ensure this is the correct URL
        const statusCount = response.data.statusCount; // Directly access the statusCount
  
        // Convert to pie data format
        const pieDataArray = Object.keys(statusCount).map(status => ({
          name: status,
        ...
        const totalUsersCount = Object.values(statusCount).reduce((acc, count) => acc + count, 0);
    ...
    <main className={styles.mainContainer}>
      ...
      <div className={styles.mainCards}>
        <div className={styles.card}>
          ...
          <h1>{totalUsers}</h1> {/* Display total users */}
        </div>
        <div className={styles.card}>
          <div className={styles.cardInner}>
            <h3>Total Groups</h3>
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={400}>
            <AreaChart data={data}>
              ...
              <YAxis />
              <Tooltip />
              <Area type="monotone" dataKey="uv" stroke="#000" fill="#8884d8" />
            </AreaChart>
          </ResponsiveContainer>
        </div>

        <div>
          <h3>User Status Distribution</h3> {/* Updated title */}
          <ResponsiveContainer width="100%" height={400}>
            <PieChart>
              <Pie
                data={pieData}
