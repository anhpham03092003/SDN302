...
const unbanUser = async (req, res, next) => {
    ...
    try {
        const result = await db.Users.updateOne({ _id: userId }, { status: 'active' });
        ...
        if (result.modifiedCount === 0) {
            console.warn("User is not banned or is already active:", userId);
            return res.status(400).json({ error: { status: 400, message: "User is not banned or is already active" } });
        }
        
        res.status(200).json({ message: "User unbanned successfully", result });
    } catch (error) {
